/*
 *  Code originated from http://www.linuxjournal.com/content/accessing-postgresql-cc
 */
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
int main(int argc, char *argv[]) {
    ...
    // prints out arguments sent to main
    /**
        for(i = 0; i < 10; i++) {
            if(i != 0)
                printf("argv[%d]: %s\n\n", i, argv[i]);
    ...
    **/
    ...
    // --------------------------------- BEGIN CONNECTION TO DATABASE ----------------------------------------------
        ...
        printf("video_path: %s\n", &video_path[0]); // Prints video_url
        //printf("table: %s\n", &table_name[0]);    // Prints table to write to
        ...
        for (row=0; row < rec_count; row++) {
            if(row == rec_count-1) {
                //printf("You wanted record %s, right?\n", PQgetvalue(res, row, 0));
        ...
        //printf("new_table: %s", &new_table[0]);
        ...
        //creates a string with a vairable inside, so we're able to dynamically send our data
        snprintf(next_command, 2000, "INSERT INTO %s(ID, name, frame, username) VALUES ('%d', '%s', '%d', '%s')", &new_table[0], id, &video_ID[0], 0, &username[0]);
        ...
        switch (child_pid_2 = fork()) {
            ...
            case 0:     // child
                ...
                //ffmpeg -i videos/mom/as23XJ.avi/as23XJ.avi -vf fps=30 videos/mom/as23XJ.avi/out%d.png
        ...
        // --------------------------------- CHANGE PERMISSIONS TO READ/WRITE ON EACH STILL-IMAGE ----------------------------------------------
              ...
              //  printf("%s", &video_path[0]);
                system("sudo -S <<< student chmod -R 777 videos/l/as23XJ.avi");
